...
Description : This program pops the smallest element from the indexed binary min heap.
*/
...
int select_min_child_index(Indexed_Binary_Min_Heap *heap, int parent)
{
    ...
    if(child_index_right >= heap->size) // No right child does'nt necessary mean there is no left child.
    {
...
int delete_indexed_binary_min_heap(Indexed_Binary_Min_Heap *heap, int key)
{
    if(heap == nullptr)
    {
        throw string {"ERROR - Invalid operation, indexed binary min heap is not valid ....."};
    ...
    heap->inverse_map->at(heap->size) = INT_MIN; // resetting the last index of the indexed binary min heap.

    return deleted_value;
}

pair<string, int> pop_indexed_binary_min_heap(map<int, string> *key_member, Indexed_Binary_Min_Heap *heap)
{
    if(heap == nullptr)
    {
        throw string {"ERROR - Invalid operation, indexed binary min heap is not valid ....."};
    ...
    if(heap->size == 0)
    {
        throw string {"ERROR - Invalid operation, indexed binary min heap is empty ....."};
    }
    ...
    delete_indexed_binary_min_heap(heap, heap->inverse_map->at(0));

    return popped_element;
}

pair<string, int> handle_pop_indexed_binary_min_heap(map<int, string> *key_member, Indexed_Binary_Min_Heap *heap)
{
    try
    {
        return pop_indexed_binary_min_heap(key_member, heap);
    }
    catch(string &ex)
    {
        cout<<ex;
...
int main()
{
    ...
    pair<string, int> result {handle_pop_indexed_binary_min_heap(&key_member, idx_bin_min_heap)};
