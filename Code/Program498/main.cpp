...
Description : This program creates an indexed binary max heap.
*/
...
struct Indexed_Binary_Max_Heap // binary indexed max heap should be a perfect tree.
{
    vector<int> *values; // stores the value of the key element(not for the index of the node).

    vector<int> *position_map; // stores the node index in the heap for the given key value.
