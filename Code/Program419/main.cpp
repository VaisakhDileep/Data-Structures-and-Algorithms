...
Description : This program finds all the tree edges in a DFS tree for an undirected graph represented using adjacency list.
*/
...
void depth_first_search(Undirected_Graph *u_graph, int node, int *visited, vector<pair<int, int>> *tree_edges)
{
    if(visited[node] == 0)
    {
        visited[node] = 1;
        ...
        while(last != nullptr)
        {
            if(visited[last->vertex] == 0)
            {
                ...
                tree_edges->push_back(pair<int, int> {last->vertex,node});

                depth_first_search(u_graph, last->vertex, visited, tree_edges);
            }
...
vector<pair<int, int>>* find_tree_edges_undirected_graph(Undirected_Graph *u_graph)
{
    if(u_graph == nullptr)
    {
        throw string {"ERROR - Invalid operation, graph is not valid ....."};
    ...
    for(int i {0}; i < u_graph->n; i++)
    {
        ...
        else
        {
            if(visited[i] == 0)
            {
                depth_first_search(u_graph, i, visited, tree_edges);
            }
        }
    }
...
vector<pair<int, int>>* handle_find_tree_edges_undirected_graph(Undirected_Graph *u_graph)
{
    try
    {
        return find_tree_edges_undirected_graph(u_graph);
    }
    catch(string &ex)
    {
        cout<<ex;
...
void display_tree_edges_undirected_graph(vector<pair<int, int>> *tree_edges)
{
...
int main()
{
    ...
    vector<pair<int, int>> *tree_edges {handle_find_tree_edges_undirected_graph(&u_graph)};

    cout<<"tree_edges(d_graph): ";
    display_tree_edges_undirected_graph(tree_edges);
    cout<<"\n";
