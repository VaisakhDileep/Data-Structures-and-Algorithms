...
Description : This program updates an element inside the indexed binary max heap.
*/
...
void update_indexed_binary_max_heap(Indexed_Binary_Max_Heap *heap, int key, int new_value)
{
    if(heap == nullptr)
    {
        throw string {"ERROR - Invalid operation, indexed binary max heap is not valid ....."};
...
void handle_update_indexed_binary_max_heap(Indexed_Binary_Max_Heap *heap, int key, int new_value)
{
    try
    {
        update_indexed_binary_max_heap(heap, key, new_value);
    }
    catch(string &ex)
    {
        cout<<ex;
...
int main()
{
    ...
    handle_update_indexed_binary_max_heap(idx_bin_max_heap, 2, 100);
