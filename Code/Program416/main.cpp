...
void depth_first_search(Directed_Graph *d_graph, int node, int *visited, vector<Exploration_Time> *node_exploration_time, vector<pair<int, int>> *cross_edges)
{
    if((node >= d_graph->n) or (d_graph->A[node] == nullptr)) // This is a leaf node.
    {
        if(visited[node] == 0)
    ...
    if(visited[node] == 0)
    {
        ...
        while(last != nullptr)
        {
            if(visited[last->vertex] == 0) // If the child is not visited, then propogate.
            {
                depth_first_search(d_graph, last->vertex, visited, node_exploration_time, cross_edges);
            }
            else // This edge does not participate in the DFS tree(could be a forward edge, back edge or a cross edge).
            {
                ...
                {
                    cross_edges->push_back(pair<int, int> {node, last->vertex});
                }
            }
...
vector<pair<int, int>>* find_cross_edges_directed_graph(Directed_Graph *d_graph)
{
    if(d_graph == nullptr)
    {
        throw string {"ERROR - Invalid operation, graph is not valid ....."};
    ...
    vector<pair<int, int>> *cross_edges {new vector<pair<int, int>> {}};
    ...
    for(int i {0}; i < d_graph->n; i++)
    {
        ...
        else
        {
            if(visited[i] == 0)
            {
                depth_first_search(d_graph, i, visited, node_exploration_time, cross_edges);
            }
        }
    }
...
vector<pair<int, int>>* handle_find_cross_edges_directed_graph(Directed_Graph *d_graph)
{
    try
    {
        return find_cross_edges_directed_graph(d_graph);
    }
    catch(string &ex)
    {
        cout<<ex;
...
void display_cross_edges_directed_graph(vector<pair<int, int>> *cross_edges)
{
    if(cross_edges == nullptr)
    {
        return ;
    }
    ...
    {
        cout<<"{"<<cross_edges->at(i).first<<", "<<cross_edges->at(i).second<<"} ";
    }
}
...
int main()
{
    ...
    vector<pair<int, int>> *cross_edges {handle_find_cross_edges_directed_graph(&d_graph)};

    cout<<"cross_edges(d_graph): ";
    display_cross_edges_directed_graph(cross_edges);
    cout<<"\n";
