...
Date        : 9, June, 2021
Description : This program counts the number of nodes in a binary tree.
*/
...
int count_nodes_binary_tree(Node *node)
{
    int count_left {}, count_right {};

    if(node != nullptr)
    {
        count_left = count_nodes_binary_tree(node->left_child);

        count_right = count_nodes_binary_tree(node->right_child);

        return count_left + count_right + 1;
    }
}

int handle_count_nodes_binary_tree(Binary_Tree *T)
{
    if(T == nullptr)
    {
...
int main()
{
    ...
    cout<<"handle_count_nodes_binary_tree(T): "<<handle_count_nodes_binary_tree(&T)<<"\n";
