...
Date		: 13, August, 2021
Description : This program finds all the brides in an undirected graph represented using adjacency list.
*/
...
void depth_first_search(Undirected_Graph *u_graph, int node, int parent, vector<int> *id, vector<int> *low_link, vector<bool> *visited, vector<vector<int>*> *bridges, int &counter)
{
	visited->at(node) = true;

	counter++;

	low_link->at(node) = id->at(node) = counter;

	Node *last {u_graph->A[node]->head};

	while(last != nullptr)
	{
		if(last->vertex == parent) // Since we have already defined the link(forward link or backward link) and don't want include both forward link and backward link.
		{
			last = last->next;
		...
		{
			depth_first_search(u_graph, last->vertex, node, id, low_link, visited, bridges, counter);

			low_link->at(node) = min(low_link->at(node), low_link->at(last->vertex));

			if(id->at(node) < low_link->at(last->vertex)) // If "low_link" of node is greater than it's previous node, then 
			{
		...
		{
			low_link->at(node) = min(low_link->at(node), id->at(last->vertex));
		}
...
vector<vector<int>*>* find_bridges_undirected_graph(Undirected_Graph *u_graph)
{
	...
	vector<int> *id {new vector<int>(max_node + 1, 0)}; // A unique id given to each node.

	vector<int> *low_link {new vector<int>(max_node + 1, 0)}; // "low_link" of a node is the smallest id reachable from the node using either forward or backward edge.
	...
	for(int i {0}; i <= max_node; i++)
	{
		...
		{
			depth_first_search(u_graph, i, -1, id, low_link, visited, bridges, counter);
		}
	}
