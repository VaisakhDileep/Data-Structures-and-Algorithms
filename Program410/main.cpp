...
Description : This program finds all the back edges in a DFS tree for a directed graph represented using adjacency list.
*/

// Back edges are the edges from the descendent node to the ancestor node.
...
void depth_first_search(Directed_Graph *d_graph, int node, int *visited, vector<Exploration_Time> *node_exploration_time, vector<pair<int, int>> *back_edges)
{
	if((node >= d_graph->n) or (d_graph->A[node] == nullptr)) // This is a leaf node.
	{
		if(visited[node] == 0)
	...
	if(visited[node] == 0)
	{
		...
		while(last != nullptr)
		{
			if(visited[last->vertex] == 0) // If the child is not visited, then propogate.
			{
				depth_first_search(d_graph, last->vertex, visited, node_exploration_time, back_edges);
			}
			else // This edge does not participate in the DFS tree(could be a forward edge, back edge or a cross edge).
			{
				if(((node_exploration_time->at(node).start_time > node_exploration_time->at(last->vertex).start_time) and (node_exploration_time->at(node).finish_time < node_exploration_time->at(last->vertex).finish_time)) or (node == last->vertex)) // We are treating self edge as a back edge.
				{
...
vector<pair<int, int>>* find_back_edges_directed_graph(Directed_Graph *d_graph)
{
	if(d_graph == nullptr)
	{
		throw string {"ERROR - Invalid operation, graph is not valid ....."};
	...
	for(int i {0}; i < d_graph->n; i++)
	{
		...
		else
		{
			if(visited[i] == 0)
			{
				depth_first_search(d_graph, i, visited, node_exploration_time, back_edges);
			}
		}
	}
...
vector<pair<int, int>>* handle_find_back_edges_directed_graph(Directed_Graph *d_graph)
{
	try
	{
		return find_back_edges_directed_graph(d_graph);
	}
	catch(string &ex)
	{
		cout<<ex;
...
void display_back_edges_directed_graph(vector<pair<int, int>> *back_edges)
{
...
int main()
{
	...
	Edge edges[19] {Edge {1, 1}, Edge {1, 3}, Edge {1, 6}, Edge {7, 2}, Edge {5, 0}, Edge {0, 1}, Edge {1, 2}, Edge {2, 3}, Edge {3, 7}, Edge {6, 7}, Edge {6, 3}, Edge {2, 6}, Edge {5, 6}, Edge {5, 2}, Edge {1, 5}, Edge {0, 4}, Edge {4, 5}, Edge {4, 1}, Edge {0, 5}};

	handle_create_directed_graph(&d_graph, edges, 19);

	cout<<"d_graph: \n";
	display_directed_graph(&d_graph);
	cout<<"\n";

	vector<pair<int, int>> *back_edges {handle_find_back_edges_directed_graph(&d_graph)};

	cout<<"back_edges(d_graph): ";
	display_back_edges_directed_graph(back_edges);
	cout<<"\n";
